...
const eventsContainer = document.querySelector(".events_admin-container") as HTMLElement;
const pendingContainer = document.querySelector(".pending-container") as HTMLElement;
...
//api functions
//need to keep track of events, so we can store updates locally (ie, made an edit, and want to edit again, but we never pinged server again)
let events = {};
let requests = {};
...
class AdminWrapper extends FirebaseDBWrapper {
    ...
    //--------- EVENTS ---------
    async getAllEvents(): Promise<Array<EventDetails>> {
...
function renderAdminEvent(event_details: EventDetails) {
    let id = event_details.doc_id;
    console.log(event_details);
    let { event_name, dateString } = event_details;
    let events_admin = createElement("div", ["events_admin"]); events_admin.id = `event_${event_details.doc_id}`;
...
function renderRequest(hour_request: HourRequest) {
    let { uid, name, hours, details, "doc_id": id } = hour_request;
    let pending = createElement("div", ["pending"]); pending.id = id;
